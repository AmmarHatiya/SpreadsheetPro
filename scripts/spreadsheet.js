window.onload = function () {
  $.ajax({
    ...
    success: function (data) {
      var grade_data = data.split(/\r?\n|\r/);
      var table_data = '<table class="Spreadsheettable">';

      // for each row: rownum
      for (rownum = 0; rownum < grade_data.length; rownum++) {
        var cell_data = grade_data[rownum].split(",");
        table_data += '<tr>';

        // for each column: columnnum
        for (var columnnum = 0; columnnum < cell_data.length; columnnum++) {
          // if first row aka studentID column
          if (rownum === 0) {
            table_data += `<th class = "colh${columnnum-1}">` + cell_data[columnnum] + '</th>';
          }
          // else
          else {
            // if in the first column
            if (columnnum === 0) {
              table_data += `<th class=rowh${rownum-1}>` + cell_data[columnnum] + '</th>'
            } else {

              table_data += `<td class=col${columnnum-1} id=row${rownum-1}>` + cell_data[columnnum] + '</td>';
